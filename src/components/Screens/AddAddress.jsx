...
// DeviceEventEmitter.addListener('check', (value)=>{
//     this.setState({ customvalue: value },()=>alert(this.state.customvalue))
// })
        this.state = {
            isSelectHome:
            ...
            areaTxtField: this.editData ? this.editData[APP_PARAMS.APARTMENT_SUIT] : undefined,
    ...
    setAddress = (flag) => {
        ...
        if (data != undefined) {
            ...
            if (flag != 'default') {
                ...
                //this.areaRef.setValue('16A, Govind Vatika, Triveni Nagar, ')
                console.warn("areaRef ref:::---",this.areaRef.value());
                
               
                this.setState({ showCurrentLocView: false, 
                    areaTxtField: data[APP_PARAMS.FORMATED_ADDRESS]
                 })
            }
        }

    }
    //16A, Govind Vatika, Triveni Nagar, Gopalpura Bypass, 10 B Scheme, Usha Vihar, Vishveshvariya Nagar, Arjun Nagar, Jaipur, Rajasthan 302018, India
    ...
    addAddressOrUpdate = () => {
        ...
        if (this.state.pinTxtField == '') {
            ...
            return;
        } else if (this.state.areaTxtField.trim() == '' || this.areaRef.value().trim() == '') {
    ...
    blurTxtField=()=>{
        ...
        this.areaRef.blur()
        this.stateRef.blur()
        this.cityRefrence.blur()
        this.landmarkRef.blur()
        this.nameRef.blur()
        this.mobileRef.blur()
        this.altrnateNoRef.blur()
    }
    ...
    serachLocation = (address) => {
       this.areaRef.setValue(address)
       this.setState({ areaTxtField: address }, () =>{
        this.areaRef.blur()
       } )
    }
    showAddressView=()=>{
    ...
    handleTextChange = (newText) => {
        this.setState({ customvalue: newText })
    };
    //API FUNCTION:--
                ...
                if (isADDEdit) {
                    if (result[APP_PARAMS.MESSAGE]) {
                        ...
                        if (this.addressIsFrom != undefined) {
                           // alert('come'+this.addressIsFrom[CONST.IS_FROM])
                            if (this.addressIsFrom[CONST.IS_FROM] == [SCREENS.CATEGORY_SUB_DETAIL]) {
                                this.props.navigation.replace(SCREENS.ORDER_SUMMARY)
                            }else if (this.addressIsFrom[CONST.IS_FROM] == [SCREENS.ORDER_SUMMARY]) {
                                if(this.props.navigation.state.params&&this.props.navigation.state.params.goBackAddAddress){
                                    this.props.navigation.state.params.goBackAddAddress(result.data)
                                  }
                                    this.props.navigation.goBack()
                            }
                        } else {
                          //  alert('result' + JSON.stringify(result))
                          if(this.props.navigation.state.params&&this.props.navigation.state.params.goBackAdress){
        ...
        return (
            <View style={{ flex: 1 }}>
                ...
                <ScrollView  keyboardShouldPersistTaps={'always'}>
                    <View>
                        ...
                        }
                        {
                           ...
                           { <Ripple style={[styles.submitBtn,{bottom:DIMENS.px_10}]} onPress={() => this.addAddressOrUpdate()}>

                            { loading ?
                             <ActivityIndicator color={colors.white} size={'small'} style={{ padding: 10 }} /> 
                                    :
                ...
                }
                {
                    ...
                    <LocationSearch modalVisible = {this.state.addressView} 
                    modalViewHide = {()=>this.hideAddressView()}
                    getAddress={(address)=>{
                       
                        this.serachLocation(address)}}/>
                }
